...
export async function saveGeneration(interactionId, data) {
    await db.collection('artbot_generations').doc(interactionId).set({
        ...data,
...
export async function getGeneration(interactionId) {
    const doc = await db.collection('artbot_generations').doc(interactionId).get();
...
export async function addReport(interactionId, reportData) {
    const reportRef = db.collection('artbot_reports').doc(interactionId);
