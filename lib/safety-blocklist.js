...
export async function executeQueenGuardScan(prompt, filterLibrary = null) {
    ...
    for (const term of allTerms) {
        ...
        if (wordRegex.test(forms.lowercase) || wordRegex.test(forms.deobfuscated)) {
            ...
            if (isHighRisk) return { approved: false, matchedTerm: `word:${term}`, score: RISK_WEIGHTS.HIGH, category };
    ...
    // If multiple distinct risk categories hit (even if not yet 100), add a bonus to reflect intent.
