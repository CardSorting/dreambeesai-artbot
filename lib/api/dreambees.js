 ...
 * @param {string} dreambeesUid - The linked DreamBees user ID.
 * @param {number} retries - Number of retry attempts.
 * @returns {Promise<{buffer: Buffer, url: string, imageId: string}>}
 */
export async function generateSingleImage(prompt, modelId, dreambeesUid, retries = 2) {
    return limit(async () => {
        ...
        try {
            ...
            logger.info(`Job queued via Backend`, { requestId, dreambeesUid });
            ...
            // 3. Fetch image from B2
            const b2Controller = new AbortController();
            const b2Timeout = setTimeout(() => b2Controller.abort(), 30000);

            const imgRes = await fetch(generationResult.imageUrl, { 
                agent: keepAliveAgent,
                signal: b2Controller.signal
            });
            
            clearTimeout(b2Timeout);
            ...
            if (err.name === 'AbortError') {
                logger.error(`API Request timed out`, { dreambeesUid, modelId });
            }
            if (retries > 0) {
                logger.info(`Retrying generation... (${retries} attempts left)`);
                return generateSingleImage(prompt, modelId, dreambeesUid, retries - 1);
            }
            throw err;
        }
    });
 ...
 * @param {string} dreambeesUid - The DreamBees user ID.
 * @param {string} requestId - The interaction/request ID.
 */
export async function registerDiscordGrid(gridUrl, prompt, modelId, dreambeesUid, requestId) {
    return limit(async () => {
        const controller = new AbortController();
