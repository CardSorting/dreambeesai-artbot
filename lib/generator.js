...
export async function performGeneration(interaction, discordUserId, prompt, modelId) {
    ...
    try {
        ...
        logger.info(`Starting generation workflow`, { discordId, discordUserId, modelId });

        // 3. Concurrent Image Generation
        const limit = pLimit(2); 
        const results = await Promise.all([
            limit(() => generateSingleImage(prompt, modelId, discordUserId)),
            limit(() => generateSingleImage(prompt, modelId, discordUserId)),
            limit(() => generateSingleImage(prompt, modelId, discordUserId)),
            limit(() => generateSingleImage(prompt, modelId, discordUserId))
        ]);
        ...
        // Push to Web App indexed by Discord Tag
        await registerDiscordGrid(gridUrl, prompt, modelId, `discord:${interaction.user.tag}`, interactionId)
        ...
        await saveGeneration(interactionId, {
            prompt, modelId, userId: discordId, discordUserId,
