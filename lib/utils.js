...
/**
 * Ensures a currency amount is rounded to exactly 2 decimal places.
 * This prevents floating point drift in user balances (Zaps).
 * 
 * @param {number|string} amount - The amount to format.
 * @returns {number} Formatted number with 2 decimal precision.
 */
export function toZapPrecision(amount) {
...
/**
 * Sanitizes input to ensure it is a finite number.
 */
...
/**
 * A sleep function that can be aborted via AbortSignal.
 * 
...
export function abortableSleep(ms, signal) {
    return new Promise((resolve, reject) => {
        if (signal?.aborted) return reject(new Error('Operation aborted during sleep'));
