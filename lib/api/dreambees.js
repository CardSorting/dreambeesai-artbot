...
export async function generateSingleImage(prompt, modelId, userId, options = {}) {
    ...
    return globalApiLimit(async () => {
        ...
        try {
            // 1. Create a placeholder in generation_queue (for analytics and compatibility)
            logger.info(`[ModalDirect] Creating queue record...`, { requestId });
            await queueRef.set({
                userId,
                prompt,
                ...
                source: 'discord_bot_direct'
            });
            logger.info(`[ModalDirect] Queue record created.`, { requestId });
            ...
            handleFailure(err);
            logger.error(`[ModalDirect] Generation failed`, { requestId, error: err.message });
