 ...
 */
export async function generateSingleImage(prompt, modelId, userId, displayName = null, retries = 3) {
    return limit(async () => {
        ...
        try {
            ...
            if (retries > 0) {
                ...
                return generateSingleImage(prompt, modelId, userId, displayName, retries - 1);
            }
            throw err;
        }
    });
 ...
 */
export async function registerDiscordGrid(gridUrl, prompt, modelId, userId, displayName = null, requestId) {
    return limit(async () => {
        const controller = new AbortController();
