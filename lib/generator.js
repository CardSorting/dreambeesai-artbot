...
export async function performGeneration(interaction, discordUserId, prompt, modelId) {
    ...
    try {
        ...
        const results = await Promise.all([
            limit(() => generateSingleImage(prompt, modelId, targetUserId, targetDisplayName)),
            limit(() => generateSingleImage(prompt, modelId, targetUserId, targetDisplayName)),
            limit(() => generateSingleImage(prompt, modelId, targetUserId, targetDisplayName)),
            limit(() => generateSingleImage(prompt, modelId, targetUserId, targetDisplayName))
        ]);
        ...
        await registerDiscordGrid(gridUrl, prompt, modelId, targetUserId, targetDisplayName, interactionId)
