...
try {
    ...
    if (saJson) {
        ...
        // Force ADC path to resolve macOS lstat bug
        process.env.GOOGLE_APPLICATION_CREDENTIALS = adcPath;
        credential = applicationDefault();
        useAdmin = true;
    } else if (process.env.K_SERVICE || process.env.NODE_ENV === 'production') {
        // Likely running on Google Cloud (Cloud Run or GCE)
        credential = applicationDefault();
        useAdmin = true;
    }
    ...
    if (useAdmin) {
        ...
        const checkPoints = [
            `saJson: ${saJson ? 'set' : 'nil'}`,
            `saPath: ${fs.existsSync(saPath) ? 'exists' : 'missing'}`,
            `GOOGLE_APPLICATION_CREDENTIALS: ${process.env.GOOGLE_APPLICATION_CREDENTIALS ? 'set' : 'nil'}`,
            `K_SERVICE: ${process.env.K_SERVICE ? 'set' : 'nil'}`,
            `GCE Environment: ${process.env.NODE_ENV === 'production' ? 'active' : 'inactive'}`
        ];
        throw new Error(`No usable Admin SDK credentials found. Checkpoints: [${checkPoints.join(', ')}]. Falling back to Web SDK...`);
    }
} catch (adminError) {
    ...
    if (process.env.FIREBASE_API_KEY) {
        try {
            ...
            if (process.env.FIREBASE_AUTH_EMAIL && process.env.FIREBASE_AUTH_PASSWORD) {
                ...
                const userCredential = await signInWithEmailAndPassword(auth, process.env.FIREBASE_AUTH_EMAIL, process.env.FIREBASE_AUTH_PASSWORD);
                currentUser = userCredential.user;
                logger.info("Firebase Web SDK authenticated as user:", process.env.FIREBASE_AUTH_EMAIL, "UID:", currentUser.uid);
            }
...
/**
 * HARDENING: Proactively verify connectivity to Firestore during startup.
 */
export async function verifyConnectivity(maxRetries = 10) {
    let attempt = 1;
    while (attempt <= maxRetries) {
        try {
            await db.collection(COLLECTIONS.HEALTH).doc('connectivity_probe').set({
                lastChecked: adminInstance.firestore.FieldValue.serverTimestamp(),
                v: '1.1.0',
                node: process.env.HOSTNAME || 'gce-hive-node'
            });
            logger.info(`[Firebase] Connectivity Verified (Attempt ${attempt})`);
            return true;
        } catch (err) {
            const delay = Math.min(Math.pow(2, attempt) * 1000, 30000); // Exponential backoff capped at 30s
            logger.warn(`[Firebase] Connectivity check failed (Attempt ${attempt}/${maxRetries}): ${err.message}. Retrying in ${delay}ms...`);
            
            if (attempt === maxRetries) {
                logger.error("[Firebase] CRITICAL: Database connectivity lost or misconfigured.");
                return false;
            }
            attempt++;
...
export { adminInstance as admin, logger, db, currentUser, COLLECTIONS };
