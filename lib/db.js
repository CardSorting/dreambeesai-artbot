 ...
 */
export async function getOrCreateDiscordUser(discordId, discordTag) {
    ...
    try {
        ...
        if (snap.exists) {
            // Update last active
            await userRef.update({ lastActive: admin.firestore.FieldValue.serverTimestamp() }).catch(() => {});
            return { uid: discordId, ...snap.data() };
        }
        ...
        logger.info(`New Discord account provisioned: @${discordTag} (${discordId}) with 100 Zaps.`);
        
        return { uid: discordId, ...newUser };
    } catch (err) {
        logger.error(`Error in getOrCreateDiscordUser for ${discordId}`, err);
        throw err;
    }
}
...
export async function cleanupStaleLocks() {
    try {
        ...
        if (err.message.includes("invalid_grant") || err.message.includes("reauth")) {
            ...
            logger.warn("[WARNING] Firestore Permission Denied for 'discord_locks'.");
            logger.warn(">>> RESOLUTION: Please ensure your Firestore Security Rules allow read/write to 'discord_locks'.");
        } else {
