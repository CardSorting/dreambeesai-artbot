...
export async function performGeneration(interaction, discordUserId, prompt, modelId, options = {}) {
    ...
    try {
        ...
        const tasks = Array.from({ length: 4 }).map((_, i) => limit(async () => {
            const result = await generateSingleImage(safePrompt, modelId, targetUserId, { logger: ctxLogger });
            const s3Filename = `generations/${interactionId}_${i}.webp`;
            const s3Url = await uploadToS3WithRetry(s3Filename, result.buffer);
        ...
        // Record Metrics
        recordGenerationMetrics(modelId, duration, true);
        ...
        recordGenerationMetrics(modelId, Date.now() - startTime, false);
