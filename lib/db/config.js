import { db, logger, COLLECTIONS } from '../firebase.js';
...
export async function getGuildConfig(guildId) {
    ...
    try {
        const doc = await db.collection(COLLECTIONS.GUILDS).doc(guildId).get();
...
export async function setGuildConfig(guildId, config) {
    await db.collection(COLLECTIONS.GUILDS).doc(guildId).set(config, { merge: true });
...
export async function getRemoteConfig() {
    ...
    try {
        const doc = await db.collection(COLLECTIONS.SYSTEM).doc('config').get();
        if (doc.exists) {
