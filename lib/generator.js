import { saveGeneration, tryLock, releaseLock, getRemainingCooldown, setCooldown, getUserByDiscordId } from './db.js';
 ...
 */
export async function performGeneration(interaction, dreambeesUid, prompt, modelId) {
    ...
    if (cooldownMs > 0) {
        ...
        return interaction.editReply({ 
            content: `⏳ **Slow down!** You're generating too fast. Please wait **${seconds}s** before starting another generation.`, 
            ephemeral: true 
        });
    }
    ...
    if (isCircuitOpen()) {
        return interaction.editReply({ content: '🔋 The backend is currently recovering. Please try again in 1 minute.', components: [] });
    }

    const lockAcquired = await tryLock(discordId);
    if (!lockAcquired) {
        return interaction.editReply({ content: '⏳ You already have an active generation!', ephemeral: true });
    }

    if (currentGlobalJobs >= MAX_GLOBAL_CONCURRENT) {
        await releaseLock(discordId);
        return interaction.editReply({ content: '🔥 Extreme system load. Please try again soon.', components: [] });
    }
    ...
    try {
        ...
        logger.info(`Debiting wallet for generation`, { discordId, dreambeesUid, totalCost });
        ...
        logger.info(`Starting generation workflow`, { discordId, dreambeesUid, modelId });

        // 3. Concurrent Image Generation
        const limit = pLimit(2); 
        const results = await Promise.all([
            limit(() => generateSingleImage(prompt, modelId, dreambeesUid)),
            limit(() => generateSingleImage(prompt, modelId, dreambeesUid)),
            limit(() => generateSingleImage(prompt, modelId, dreambeesUid)),
            limit(() => generateSingleImage(prompt, modelId, dreambeesUid))
        ]);
        ...
        // 5. Grid Storage & Registration
        const gridFilename = `discord-grids/${interactionId}.webp`;
        const gridUrl = await uploadToS3WithRetry(gridFilename, gridBuffer);
        await registerDiscordGrid(gridUrl, prompt, modelId, dreambeesUid, interactionId);

        // 6. Build Discord Response
        const embeds = [createGenerationEmbed(prompt, modelId, dreambeesUid)];

        // 7. Save to Local Persistence
        await saveGeneration(interactionId, {
            prompt, modelId, userId: discordId, dreambeesUid,
        ...
        await interaction.editReply({ 
            ...
            components: [createNavRow(dreambeesUid), createUpscaleRow(interactionId, imageIds), createModRow(interactionId)]
        });
