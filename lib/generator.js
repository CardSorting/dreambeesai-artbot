...
export async function performGeneration(interaction, discordUserId, prompt, modelId) {
    ...
    try {
        ...
        const isTechnicalError = error.message.includes('not a function') || error.message.includes('index') || error.message.includes('snapshot');
        const displayErrorMessage = isTechnicalError ? Hive.Voice.failure : `❌ **Generation Failed:** ${error.message}`;

        await interaction.editReply({ 
            content: `${displayErrorMessage}. ${zapsDebited > 0 ? 'Your Zaps have been automatically refunded.' : ''}`, 
            components: [] 
        });
    } finally {
        currentGlobalJobs--;
