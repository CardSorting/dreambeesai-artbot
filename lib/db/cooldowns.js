...
export async function getRemainingCooldown(userId) {
    const doc = await db.collection(COLLECTIONS.COOLDOWNS).doc(userId).get();
    if (!doc.exists) return 0;
    const data = doc.data();
    const remaining = (data.endsAt?.toDate?.() || data.endsAt) - new Date();
...
export async function setCooldown(userId, durationMs) {
    ...
    await db.collection(COLLECTIONS.COOLDOWNS).doc(userId).set({
        endsAt,
