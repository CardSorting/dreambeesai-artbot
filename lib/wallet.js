...
import { normalizeDate, toZapPrecision } from './utils.js';
...
export class Wallet {
    ...
    static async debit(uid, amount, requestId, metadata = {}, currency = 'zaps') {
        ...
        const transactionRef = db.collection(COLLECTIONS.TRANSACTIONS).doc(requestId);
        const { autoQueue } = metadata;
        ...
        return await db.runTransaction(async (t) => {
            ...
            const currentBalance = toZapPrecision(userData[currency] || 0); 

            const safeAmount = toZapPrecision(Math.max(0, parseFloat(amount) || 0));
            if (safeAmount > 0 && currentBalance < safeAmount) {
                throw new InsufficientFundsError(currency, safeAmount, currentBalance);
            }
            const newBalance = toZapPrecision(Math.max(0, currentBalance - safeAmount));
            ...
            // ATOMIC PERFORMANCE HARDENING: Unified pre-flight state creation
            if (autoQueue) {
                const queueRef = db.collection(autoQueue.collection).doc(requestId);
                t.set(queueRef, {
                    ...autoQueue.data,
    ...
    static async credit(uid, amount, requestId, metadata = {}, currency = 'zaps') {
        if (!uid || !requestId) throw new Error('User ID and requestId are required for credit transactions.');
    ...
    static async refund(originalRequestId, reason = 'System Refund') {
        ...
        const transactionRef = db.collection(COLLECTIONS.TRANSACTIONS).doc(originalRequestId);
        ...
        return await db.runTransaction(async (t) => {
            ...
            const userData = userDoc.data() || {};
            const currentBalance = toZapPrecision(userData[currency] || 0);
            const refundAmount = toZapPrecision(Math.max(0, amount || 0));
            const newBalance = toZapPrecision(Math.max(0, currentBalance + refundAmount));
            ...
            const refundRef = db.collection(COLLECTIONS.TRANSACTIONS).doc(refundRequestId);
            t.set(refundRef, {
                userId: uid,
                type: 'credit',
                currency,
    ...
    static async claimDaily(uid, context = {}) {
        ...
        try {
            ...
            return await db.runTransaction(async (transaction) => {
                ...
                const userData = userDoc.data();
                const lastClaim = normalizeDate(userData.lastFreeClaimAt);
                ...
                const currentBalance = toZapPrecision(userData.zaps || 0);
                const safeReward = toZapPrecision(Math.max(0, totalReward || 0));
                const newBalance = toZapPrecision(Math.max(0, currentBalance + safeReward));
                ...
                transaction.set(transactionRef, {
                    userId: uid, type: 'credit', currency: 'zaps', amount: totalReward, previousBalance: currentBalance, newBalance, requestId, 
                    status: 'completed', source: 'daily_claim', metadata: { streak: newStreak, baseAmount, bonusAmount, ...context },
                    timestamp: admin.firestore.FieldValue.serverTimestamp(), createdAt: now.toISOString()
                });
