...
export function createGenerationEmbed(prompt, modelId) {
    return new EmbedBuilder()
        ...
        .addFields(
            ...
            { name: '⚡ Cost', value: '4 Zaps', inline: true }
        )
        .setImage('attachment://generation.png')
        .setFooter({ text: 'DreamBees Alchemist • Independent Discord Account' });
}
...
export class ThreadedInteraction {
    ...
    async deferReply(options = {}) {
        ...
        this.threadMessage = await this.thread.send({ 
            content: `🎨 **DreamBees is thinking...** (Executing \`/${jobName}\`)` 
        });
