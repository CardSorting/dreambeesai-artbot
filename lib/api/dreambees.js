 ...
 */
export async function generateSingleImage(prompt, modelId, userId, retries = 2) {
    return limit(async () => {
        ...
        try {
            ...
            if (retries > 0) {
                ...
                return generateSingleImage(prompt, modelId, userId, retries - 1);
            }
            throw err;
        }
    });
 ...
 */
export async function registerDiscordGrid(gridUrl, prompt, modelId, userId, requestId) {
    return limit(async () => {
        const controller = new AbortController();
