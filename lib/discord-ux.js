...
export class HiveInteraction {
    ...
    async reply(options) {
        ...
        try {
            if (payload.ephemeral) this.isEphemeral = true;
    ...
    async editReply(options) {
        ...
        try {
            ...
            // If we want a public response but deferred ephemerally, we must use followUp
            if (this.isEphemeral && !payload.ephemeral) {
                // Acknowledge the ephemeral message so it doesn't stay "Thinking..."
                if (this.interaction.deferred && !this.interaction.replied) {
                    await this.interaction.editReply({ content: '✅ **Harvesting Complete!**' }).catch(() => {});
                }
            ...
            if (e.code === 10062 || e.code === 50027) {
                ...
                if (this.interaction.channel) {
                    ...
                    if (payload.content) {
                        fallbackPayload.content = `⚠️ **Update:** The original interaction expired, but your result is ready!\n\n${payload.content}`;
                    } else {
                        fallbackPayload.content = `⚠️ **Update:** The original interaction expired, but your result is ready!`;
                    }
                    return await this.interaction.channel.send(fallbackPayload).catch(() => {});
                }
            }
            throw e;
        }
